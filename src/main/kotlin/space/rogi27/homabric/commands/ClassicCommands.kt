    ...
    fun init() {
        if (HomabricConfig.areClassicCommandsEnabled()) {
            CommandRegistrationCallback.EVENT.register(CommandRegistrationCallback { dispatcher, _, _ ->
                dispatcher.register(
                    CommandManager.literal("sethome").requires(Permissions.require("homabric.base.set", 0)).then(
                                CommandManager.argument("home", StringArgumentType.word()).executes((BaseCommands::set))
                            ).executes(BaseCommands::set)
                )
                dispatcher.register(
                    CommandManager.literal("removehome").requires(Permissions.require("homabric.base.remove", 0)).then(
                        CommandManager.argument("home", StringArgumentType.word()).suggests(Completables::suggestPlayerHomes).executes((BaseCommands::remove))
                    )
                )
                dispatcher.register(
                    CommandManager.literal("playerhome").requires(Permissions.require("homabric.base.others", 0)).then(
                        CommandManager.argument("player", EntityArgumentType.player()).then(
                            CommandManager.argument("home", StringArgumentType.word()).suggests(Completables::suggestAllowedHomes).executes((BaseCommands::teleportToAllowed))
                        )
                    )
                )
                dispatcher.register(
                ...
                dispatcher.register(
                    CommandManager.literal("allowhome").requires(Permissions.require("homabric.base.allow", 0)).then(
                                CommandManager.argument("player", EntityArgumentType.player()).then(
                                            CommandManager.argument("home", StringArgumentType.word()).suggests(Completables::suggestPlayerHomes).executes((BaseCommands::allowHome))
                                        )
                            )
                )
                dispatcher.register(
                    CommandManager.literal("disallowhome").requires(Permissions.require("homabric.base.disallow", 0)).then(
                        CommandManager.argument("player", EntityArgumentType.player()).then(
                                    CommandManager.argument("home", StringArgumentType.word()).suggests(Completables::suggestPlayerHomes).executes((BaseCommands::disallowHome))
                                )
                    )
                )
            })
